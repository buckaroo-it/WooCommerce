        ...
        <div class="form-row form-row-wide bk-paybybank-selector">
            <?php foreach ($issuers as $key => $issuer) : ?>
                <div class="custom-control custom-radio bank-control">
                    <input name="buckaroo-paybybank-radio-issuer" type="radio" <?php if(isset($issuer["selected"]) && $issuer["selected"] === true) {?> checked <?php } ?> id="radio-bankMethod<?php echo esc_attr($key); ?>" value="<?php echo esc_attr($key); ?>" class="custom-control-input bank-method-input bk-paybybank-radio">
