<?php
$previous_captures = $this->get_previous_captures();
$items_to_capture = $this->get_available_to_capture_by_type($this);
    ...
    <ul>
        ...
        foreach ($items_to_capture['line_item'] as $item) {
            ...
            <li class="bk-to-capture">
                ...
                <div class="cost bk-auto m-h-15">
                    <?php
                    echo wc_price($item->get_unit_price(), ['currency' => $this->get_order_details()->get_currency()])
                    ?>
                </div>
                <div class="qty-max bk-auto m-h-15">
                ...
                <div class="qty bk-auto m-h-15">
                    <input data-item-id="<?php echo $item->get_line_item_id(); ?>"
                            data-unit-price="<?php echo $item->get_unit_price(); ?>" type="number" step="1" min="0"
                            max="<?php echo $item->get_quantity(); ?>" value="<?php echo $item->get_quantity(); ?>">
                </div>
                <div class="amount bk-auto m-h-15" style="margin-right:0">
                    <?php
                    echo wc_price($item->get_total_amount(), ['currency' => $this->get_order_details()->get_currency()])
                    ?>
                </div>
            </li>
            <?php
    ...
    <ul>
        ...
        foreach ($items_to_capture['shipping'] as $item) {
            ...
            <li class="bk-to-capture">
                ...
                <div class="qty bk-auto m-h-15">
                    <input data-item-id="<?php echo $item->get_line_item_id(); ?>"
                            data-unit-price="<?php echo $item->get_unit_price(); ?>" type="checkbox" checked="checked">
                </div>
                <div class="amount bk-auto m-h-15" style="margin-right:0">
                    <?php
                    echo wc_price($item->get_unit_price(), ['currency' => $this->get_order_details()->get_currency()])
        ...
        foreach ($previous_captures as $capture) {
            ...
            <li class="bk-previous-capture">
                ...
                <div class="bk-refund-amount-wrap">
                    <div class="amount">
                        <?php
                        echo wc_price(str_replace(',', '.', $capture->get_total_amount()), ['currency' => $capture->get_currency()])
                        ?>
                    </div>
                    <?php
