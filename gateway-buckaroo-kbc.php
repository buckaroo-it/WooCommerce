...
class WC_Gateway_Buckaroo_KBC extends WC_Gateway_Buckaroo {
    ...
    function __construct()
    {
        ...
        $this->title = 'KBC/CBC';
        $this->icon         = apply_filters('woocommerce_buckaroo_kbc_icon', plugins_url('library/buckaroo_images/kbc.png', __FILE__));
        ...
        if ( version_compare( WOOCOMMERCE_VERSION, '2.0.0', '<' ) ) {
            ...
            add_action( 'woocommerce_api_wc_gateway_buckaroo_kbc', array( $this, 'response_handler' ) );
            $this->notify_url   = add_query_arg('wc-api', 'WC_Gateway_Buckaroo_KBC', $this->notify_url);
        }
    }
    ...
    public function process_refund($order_id, $amount = null, $reason = ''){
        ...
        try {
            $kbc->checkRefundData($orderRefundData);
            $response = $kbc->Refund();
        } catch (Exception $e) {
    ...
    function process_payment($order_id) {
        ...
        $kbc->invoiceId =  getUniqInvoiceId($order_id);
