...
class HostedFieldsController {
    ...
    public static function getToken() {
        ...
        if ( json_last_error() !== JSON_ERROR_NONE ) {
            wp_send_json( array( 'error' => __( 'Error: Invalid response from server.', 'wc-buckaroo-bpe-gateway' ) ), 500 );
            return false;
        }
        ...
        if ( isset( $data['access_token'] ) ) {
            ...
            wp_send_json( array( 'error' => __( $error_message, 'wc-buckaroo-bpe-gateway' ) ), 500 );
            return false;
        }
    }
}
