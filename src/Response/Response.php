...
abstract class Response
{
    ...
    public function __construct($data = null)
    {
        ...
        if ($this->isHttpRequest()) {
            Logger::log('Type: HTTP');
            Logger::log('POST', print_r(wc_clean($_POST), true));
        } else {
            Logger::log('Type: SOAP');
            if (!is_null($data)) {

                if ($data[0] != false) {
                    Logger::log('Data[0]: ', print_r($data[0], true));
                }

                if ($data[1] != false) {
                    Logger::log('Data[1]: ', $data[1]->saveHTML());
                }

                if ($data[2] != false) {
                    Logger::log('Data[2]: ', $data[2]->saveHTML());
                }
            }
        }
    ...
    protected function _verifySignature()
    {
        ...
        $certificatesDir = __DIR__ . '/../../' . Config::CERTIFICATE_PATH;
        ...
        // get the public key
        if (!file_exists($certificatePath)) {
            Logger::log($certificatePath . ' do not exists');
        }
    ...
    protected function _calculateSignature()
    {
        ...
        $signatureString .= Config::get('BUCKAROO_SECRET_KEY', $transaction_method);
