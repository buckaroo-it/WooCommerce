...
export default class ApplePay {
	...
	init() {
		BuckarooSdk.ApplePay.checkApplePaySupport(
			this.store_info.merchant_id
		).then( ( is_applepay_supported ) => {
			if ( is_applepay_supported ) {
				const cart_items = this.getItems();
				const shipping_methods = this.woocommerce.getShippingMethods(
					this.country_code
				);
				const first_shipping_item =
					this.getFirstShippingItem( shipping_methods );

				const all_items =
					first_shipping_item !== null
				...
				if ( shipping_methods.length > 0 ) {
					this.selected_shipping_method =
						shipping_methods[ 0 ].identifier;
					this.selected_shipping_amount =
						shipping_methods[ 0 ].amount;
				}
				...
				const applepay_options =
					new BuckarooSdk.ApplePay.ApplePayOptions(
				...
				const applepay_payment =
					new BuckarooSdk.ApplePay.ApplePayPayment(
	...
	processApplepayCallback( payment ) {
		...
		if ( authorization_result.status === ApplePaySession.STATUS_SUCCESS ) {
			...
			const errors = authorization_result.errors
