...
export default class Woocommerce {
	...
	getItems( country_code ) {
		if (
			jQuery( '.applepay-button-container' ).hasClass( 'is-detail-page' )
		) {
	...
	getShippingMethods( country_code ) {
		const product_params = ( () => {
			if (
				jQuery( '.applepay-button-container' ).hasClass(
					'is-detail-page'
				)
			) {
				const current_shown_product = this.getCurrentShownProduct();
	...
	canOrderAmount() {
		...
		const current_amount = parseInt(
			jQuery( '.cart .quantity input.qty' ).val()
		);
		const max_amount = parseInt(
			jQuery( '.cart .quantity input.qty' ).attr( 'max' )
		);
		if ( isNaN( max_amount ) ) {
