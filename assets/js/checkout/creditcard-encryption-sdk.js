...
( function ( BuckarooClientSideEncryption ) {
	...
	( function ( V001 ) {
		...
		V001.validateCardNumber = function ( cardNumberString, cardService ) {
			if (
				typeof cardNumberString === 'undefined' ||
			...
			switch ( cardService.toLowerCase() ) {
				...
				case 'postepay':
				case 'mastercard':
					return /^(5[1-5]|2[2-7])[0-9]{14}$/.test(
						cardNumberString
					);
				case 'bancontactmrcash':
				case 'bancontact':
					return /^(4796|6060|6703|5613|5614)[0-9]{12,15}$/.test(
						cardNumberString
					);
				case 'maestro':
				...
				case 'cartebancaire':
				case 'cartasi':
					return /^((5[1-5]|2[2-7])[0-9]{14})|(4[0-9]{12}(?:[0-9]{3})?)$/.test(
						cardNumberString
					);
				default:
		...
		const base64EncodeUint8Array = function ( uint8Array ) {
			return btoa( String.fromCharCode.apply( null, uint8Array ) );
		};
		// Decodes a Base64 string to an Uint8 array.
		const base64Decode = function ( base64String ) {
			return atob( base64String )
				.split( '' )
				.map( ( c ) => c.charCodeAt( 0 ) );
		};
		// Creates an Uint8Array of the given card data
		const cardDataToUint8Array = function (
			cardNumber,
			year,
			month,
			cvc,
			...
			const encryptableUtf8String = unescape(
				encodeURIComponent( encryptableString )
			);
		...
		//
		V001.encryptCardDataOther = function (
			cardNumber,
			year,
			month,
			cvc,
			...
			const encryptableUint8Array = cardDataToUint8Array(
				cardNumber,
				year,
				month,
				cvc,
		...
		//
		// Usage:
		V001.encryptCardDataIE = function (
			cardNumber,
			year,
			month,
			cvc,
			...
			const importOperation = cryptoSubtle.importKey(
				Variables.keyFormat,
				keyArray,
				Variables.algorithmParams,
				true,
			...
			importOperation.oncomplete = function ( event ) {
				...
				const encryptOperation = cryptoSubtle.encrypt(
					Variables.algorithmParams,
					publicKey,
					encryptableUint8Array.buffer
				);
				...
				encryptOperation.oncomplete = function ( e ) {
					const encryptedUint8Array = new Uint8Array(
						e.target.result
					);
					const encryptedBase64String =
						base64EncodeUint8Array( encryptedUint8Array );
					const encryptedCardData =
						Variables.version + encryptedBase64String;
					callback( encryptedCardData );
				};
			};
		};
		V001.encryptCardData = function (
			cardNumber,
			year,
			month,
			cvc,
		...
		( V001 =
			BuckarooClientSideEncryption.V001 ||
			( BuckarooClientSideEncryption.V001 = {} ) )
	);
