class BuckarooPayByBank {
	...
	init() {
		...
		jQuery( 'body' ).on( 'change', '.bk-paybybank-radio', () => {
			...
			jQuery(
				'.bk-paybybank-real-value, .buckaroo-paybybank-select'
			).val( value );
			self.setLogo();
		} );
	...
	setRadioSelectedFromRealValue() {
		const selected = jQuery(
			`.bk-paybybank-radio[value="${ jQuery(
				'.bk-paybybank-real-value'
			).val() }"]`
		);
		if ( selected.length ) {
			jQuery( '.bk-toggle' )
	...
	setLogo() {
		...
		if ( buckaroo_global.payByBankLogos && code && code.length ) {
			if ( buckaroo_global.payByBankLogos[ code ] ) {
				jQuery(
					'.payment_method_buckaroo_paybybank > label > img'
				).prop( 'src', buckaroo_global.payByBankLogos[ code ] );
			}
		}
	}
