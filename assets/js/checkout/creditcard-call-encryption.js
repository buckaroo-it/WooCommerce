...
class BuckarooValidateCreditCards {
	...
	submit() {
		...
		const cardNumberValid =
			BuckarooClientSideEncryption.V001.validateCardNumber( cardNumber );
		const cvcValid = BuckarooClientSideEncryption.V001.validateCvc( cvc );
		const cardHolderNameValid =
			BuckarooClientSideEncryption.V001.validateCardholderName(
				cardHolderName
			);
		const expirationYearValid =
			BuckarooClientSideEncryption.V001.validateYear( expirationYear );
		const expirationMonthValid =
			BuckarooClientSideEncryption.V001.validateMonth( expirationMonth );
		if (
			cardNumberValid &&
			cvcValid &&
			cardHolderNameValid &&
			...
			this.constructor.getEncryptedData(
				cardNumber,
				expirationYear,
				expirationMonth,
				cvc,
