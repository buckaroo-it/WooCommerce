...
function TermsAndConditionsCheckbox( {
	...
	const getTermsUrl = ( country, isB2B = false ) => {
		const baseUrl =
			'https://documents.riverty.com/terms_conditions/payment_methods/';
		const languageMap = {
			DE: 'de_de',
			NL: 'nl_nl',
			BE: 'be_nl',
	...
	const fieldName =
		paymentMethod === 'buckaroo_afterpaynew'
			? 'buckaroo-afterpaynew-accept'
			: paymentMethod === 'buckaroo_afterpay'
			? 'buckaroo-afterpay-accept'
			: paymentMethod;
	const { country } = billingData;
	let labelText = __(
		'Accept Riverty conditions:',
	...
	if ( paymentMethod === 'buckaroo-billink' ) {
		...
		termsUrl =
			'https://www.billink.nl/app/uploads/2021/05/Gebruikersvoorwaarden-Billink_V11052021.pdf';
	}
