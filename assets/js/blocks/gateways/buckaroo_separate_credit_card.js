...
function SeparateCreditCard( { onStateChange, gateway } ) {
	...
	const initialState = {
		[ `${ paymentMethodId }-creditcard-issuer` ]: paymentMethodId.replace(
	...
	useEffect( () => {
		updateFormState(
			...
			initialState[ `${ paymentMethodId }-creditcard-issuer` ]
		);
	}, [ `${ paymentMethodId }-creditcard-issuer` ] );
	...
	const handleEncryption = async () => {
		...
		try {
			...
			onStateChange( {
				...formState,
				[ `${ paymentMethodId }-encrypted-data` ]: encryptedData,
			} );
		} catch ( error ) {
		...
		<div>
			<div className="method--bankdata">
				<input
					...
					value={
						formState[ `${ paymentMethodId }-creditcard-issuer` ]
					}
				/>
				{ creditCardIsSecure === true && (
					<div>
						...
						<div className="form-row">
							<label
								className="buckaroo-label"
								htmlFor={ `${ paymentMethodId }-cardyear` }
							>
								{ __(
									'Expiration Year:',
									'wc-buckaroo-bpe-gateway'
