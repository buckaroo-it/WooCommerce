...
class BuckarooResponseFactory {
    ...
    private static function getPaymentMethod($data = null) {
        ...
        else if (isset($_POST['brq_payment_method']) && is_string($_POST['brq_payment_method'])) { //brq_payment_method - The service code identifying the type of payment that has occurred.
            $paymentMethod = sanitize_text_field($_POST['brq_payment_method']);
        } // HTTP ???
        else if (isset($_POST['brq_transaction_method']) && is_string($_POST['brq_transaction_method'])) { //brq_ transaction_method The service code identifying the type of transaction that has occurred. (If no payment has occurred, for example when a customer cancels on the redirect page.
