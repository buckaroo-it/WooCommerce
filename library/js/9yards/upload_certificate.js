...
buckarooAdmin = {
  testButton: function () {
    ...
    buckarooTestButton.on("click", function () {
      let website_key = jQuery(
        '[name^="woocommerce_buckaroo_"][name$="_merchantkey"]',
      ).val();
      let secret_key = jQuery(
        '[name^="woocommerce_buckaroo_"][name$="_secretkey"]',
      ).val();
      jQuery.post(
        ajaxurl,
        {
  ...
  credicardToggleSelect: function () {
    ...
    var self = this;
    jQuery("#woocommerce_buckaroo_creditcard_creditcardmethod").on(
      "change",
      function () {
  ...
  in3FrontEndLabel() {
    jQuery("#woocommerce_buckaroo_in3_api_version").on("change", function () {
      ...
      const label =
        apiVersion === buckaroo_php_vars.version2
          ? buckaroo_php_vars.in3_v2
          : buckaroo_php_vars.in3_v3;
