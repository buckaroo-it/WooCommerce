jQuery(document).ready(function () {
  ...
  jQuery(
    ".capture input.capture_line_total, .capture input.capture_line_tax",
  ...
  var doCapture = function () {
    if (
      window.confirm(
        "Are you sure you wish to process this capture? This action cannot be undone.",
      )
    ) {
      ...
      jQuery(".capture input.capture_order_item_qty").each(
        function (index, item) {
          if (jQuery(item).closest("tr").data("order_item_id")) {
            if (item.value) {
              line_item_qtys[jQuery(item).closest("tr").data("order_item_id")] =
                item.value;
            }
          }
        },
      ...
      jQuery(".capture input.capture_line_total").each(function (index, item) {
        if (jQuery(item).closest("tr").data("order_item_id")) {
          line_item_totals[jQuery(item).closest("tr").data("order_item_id")] =
            item.value;
        }
      });
        ...
        if (jQuery(item).closest("tr").data("order_item_id")) {
          ...
          if (
            !line_item_tax_totals[
              jQuery(item).closest("tr").data("order_item_id")
            ]
          ) {
            line_item_tax_totals[
              jQuery(item).closest("tr").data("order_item_id")
      ...
      if (0 < unit_total_tax) {
        $refund_line_total_tax
          .val(
            parseFloat(
              accounting.formatNumber(
                BK_price_round(unit_total_tax * refund_qty),
                woocommerce_admin_meta_boxes.rounding_precision,
                "",
              ),
            )
  ...
  var inputChanged = function () {
    ...
    var $items = jQuery(".woocommerce_order_items").find(
      "tr.capture-item, tr.fee, tr.shipping",
    );

    $items.each(function () {
      var $row = jQuery(this);
      var refund_cost_fields = $row.find(
        ".capture input:not(.capture_order_item_qty)",
      );
  ...
  var amountChanged = function () {
    var total = accounting.unformat(
      jQuery(".wc-order-capture-items #capture_amount").val(),
      woocommerce_admin.mon_decimal_point,
    );
    jQuery("button .wc-order-capture-amount .amount").text(
  ...
  var captureForm = function () {
    ...
    jQuery("div.wc-order-capture-data-row-toggle")
      .not("div.wc-order-capture-items")
      .slideUp();
    jQuery("div.wc-order-capture-totals-items").slideUp();
    jQuery("#buckaroo-order-capture").find("div.capture").show();
    jQuery(
      ".wc-order-capture-edit-line-item .wc-order-edit-line-item-actions",
    ).hide();
    return false;
  };
  ...
  var cancelForm = function () {
    jQuery("div.wc-order-capture-data-row-toggle")
      .not("div.wc-capture-bulk-actions")
      .slideUp();
